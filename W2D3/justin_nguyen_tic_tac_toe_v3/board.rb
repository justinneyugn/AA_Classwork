class Board
    def initialize(n)
        @grid = Array.new(n) { |i| Array.new(n) { |i| "_"}}
        @size = n
    end
    ...
    def valid?(position)
        ...
        return true if (position[0] < @size && position[0] >= 0) && (position[1] < @size && position[1] >= 0)
    ...
    def win_row?(mark)
        @grid.each { |sub| return true if sub.count { |ele| ele == mark} == @size }
    ...
    def legal_positions
        ...
        @grid.each_with_index do |sub, i|
            @grid.each_with_index { |ele, j| legal << [j,i] if @grid[j][i] == "_" }
        end
